...
{
	if (g_hook_n >= sizeof(g_hooks) / sizeof(g_hooks[0]))
		return;
	size_t cpy = strlen(cmd_upper);
	if (cpy >= sizeof(g_hooks[0].name))
		cpy = sizeof(g_hooks[0].name) - 1;
	memcpy(g_hooks[g_hook_n].name, cmd_upper, cpy);
	g_hooks[g_hook_n].name[cpy] = '\0';
	g_hooks[g_hook_n].fn = fn;
	g_hooks[g_hook_n].user = user;
	g_hook_n++;
}

enum roxy_hook_result roxy_hook_apply(const struct roxy_cmd *cmd,
				      roxy_rewrite_t *out)
{
	// 1) Wildcard hook (e.g., global logger) if present
	for (size_t i = 0; i < g_hook_n; i++) {
		if (strcmp(g_hooks[i].name, "*") == 0) {
			enum roxy_hook_result r =
			    g_hooks[i].fn(cmd, out, g_hooks[i].user);
			if (r != ROXY_PASS)
				return r;
			break;
		}
					  ...
					  roxy_rewrite_t *out UNUSED, void *user UNUSED)
{
	...
	for (size_t i = 0; i < cmd->argc && i < 6; i++) {
		...
		if (off + n + 1 >= sizeof(preview))
			break;
		off +=
		    (size_t)snprintf(preview + off, sizeof(preview) - off,
				     "%s%.*s", (i ? " " : ""), (int)n,
				     cmd->argv[i].ptr);
	}
	LOGI("[HOOK:*] %s argc=%zu%s%.*s", cmd->cmd, cmd->argc,
	     off ? " preview=" : "", (int)off, preview);
	return ROXY_PASS;
}
					   ...
					   roxy_rewrite_t *out UNUSED, void *user UNUSED)
{
	...
	LOGI("CMD %s argc=%zu%s%.*s", cmd->cmd, cmd->argc,
	     off ? " preview=" : "", (int)off, preview);
	return ROXY_PASS;
}
