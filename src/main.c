...
{
	const char *v;
	if ((v = getenv("ROXY_HOST")) && *v)
		cfg->roxy_host = v;
	if ((v = getenv("ROXY_PORT")) && *v)
		cfg->roxy_port = (unsigned short)atoi(v);
	if ((v = getenv("REDIS_HOST")) && *v)
		cfg->redis_host = v;
	if ((v = getenv("REDIS_PORT")) && *v)
		cfg->redis_port = (unsigned short)atoi(v);
}
...
int main(int argc, char **argv)
{
	...
	while ((c = getopt_long(argc, argv, "vh", opts, nullptr)) != -1) {
		switch (c) {
		case 1000:
			cfg.roxy_host = optarg;
			break;
		case 1001:
			cfg.roxy_port = (unsigned short)atoi(optarg);
			break;
		case 1002:
			cfg.redis_host = optarg;
			break;
		case 1003:
			cfg.redis_port = (unsigned short)atoi(optarg);
			break;
		case 'v':
			if (cfg.log_level > 0)
				cfg.log_level--;
