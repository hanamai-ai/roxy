...
int resp_parse_request(const char *buf, const size_t len, size_t *frame_len,
		       struct roxy_cmd *out)
{
	if (len == 0)
		return 0;
	if (buf[0] == '*') {
		size_t pos = 0;
		size_t used;
		const long num_items =
		    parse_crlf_number(buf + 1, len - 1, &used);
		if (num_items == LONG_MIN)
			return 0;
		if (num_items <= 0 || num_items > ROXY_MAX_ARGS)
			return -1;
		pos = 1 + used;
		out->argc = 0;
		for (long i = 0; i < num_items; ++i) {
			...
			size_t u2;
			long blen =
			    parse_crlf_number(buf + pos + 1, len - pos - 1,
					      &u2);
			if (blen == LONG_MIN)
				return 0;
			if (blen < 0)
		...
		const size_t copy =
		    n0 < sizeof(out->cmd) - 1 ? n0 : sizeof(out->cmd) - 1;
	...
	out->argc = 0;
	size_t i = 0;
	while (&buf[i] < cr && out->argc < ROXY_MAX_ARGS) {
		while (&buf[i] < cr && (buf[i] == ' ' || buf[i] == '\t'))
			i++;
